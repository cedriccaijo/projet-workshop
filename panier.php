...
$id_Panier   = isset($_GET['id_article'])   ? $_GET['id_Panier']   : null;
$nomProduit  = isset($_GET['nom_article'])  ? $_GET['nomProduit']  : null;
$prixProduit = isset($_GET['prix_article']) ? $_GET['prixProduit'] : '?';
$qteProduit  = isset($_GET['qte_article'])  ? $_GET['qteProduit']  : 1;
...
if (isset($_GET['ajouter'])){ // Ajouter un nouvel article
  $_SESSION['panier'][$id_article]['nom']  = $nomProduit;
  $_SESSION['panier'][$id_article]['prix'] = $prixProduit;
  $_SESSION['panier'][$id_article]['qte']  = $qteProduit;
} 
else if (isset($_GET['modifier']))  $_SESSION['panier'][$id_Panier]['qte'] = $qteProduit; // Modifier la quantité achetée
else if (isset($_GET['supprimer']))  unset($_SESSION['panier'][$id_Panier]); // Supprimer un article du panier
